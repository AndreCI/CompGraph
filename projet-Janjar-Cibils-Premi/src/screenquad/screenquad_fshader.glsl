...
void main() {
    ...
    vec2 debugTex = vec2(1,0);
    float N = 15;
   vec4 tempColor = texture(colorTex, uv);
    for(float i =0; i<N/5; i+=0.2)//0.2 et N/5 pour normaliser a la fin
    {
        tempColor += texture(colorTex, uv - texture(velocityTex,uv).xy*i);
    }
    color = tempColor/vec4(N);
}
